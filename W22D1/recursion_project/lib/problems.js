...
function reverseString(str) {
  ...
  let new_arr;
  let ele = arr.pop();
  new_arr = ele + reverseString(arr.join(""));
  return new_arr;
}
...
function fileFinder(directories, targetFile) {
  ...
  Object.keys(directories).forEach(sub => {
    if (sub[0] === '/') {
      if (fileFinder(directories[sub], targetFile)) {
        return true;
      }
    }
  })
